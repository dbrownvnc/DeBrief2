...
def load_config():
    ...
    config = {
        ...
        "news_history": {} # [NEW] 클라우드에 저장될 뉴스 기록
    }
...
def start_background_worker():
    ...
    def run_bot_system():
        ...
        try:
            ...
            try: bot.send_message(chat_id, "🤖 DeBrief V50 가동\n클라우드 동기화 모드.")
            except: pass

            @bot.message_handler(commands=['start', 'help'])
            def start_cmd(m): 
                bot.reply_to(m, "🤖 *DeBrief V50*\n/earning, /summary, /eco, /news, /sec, /p, /list", parse_mode='Markdown')
            ...
            def analyze_ticker(ticker, settings, token, chat_id):
                ...
                try:
                    # [V50] 뉴스 중복 방지 (JSONBin에 저장된 히스토리 사용)
                    if settings.get('뉴스') or settings.get('SEC'):
                        # 설정 파일 다시 로드 (최신 히스토리 반영)
...
st.markdown("<h3 style='color: #1A73E8;'>📡 DeBrief Cloud (V50)</h3>", unsafe_allow_html=True)
